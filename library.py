...
def return_book(book_id: int, member_id: int):
    ...
    if not book_data or not member_data:
        raise HTTPException(404)
    if book_data.is_available or not book_data.borrowed_by_member_id == member_id:
        raise HTTPException(400)
    update_book = base_models.UpdateBook(is_available=True, borrowed_by_member_id=None)
    book_manager.update_book(book_id, update_book)
